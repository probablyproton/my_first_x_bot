...
## Rules
- Sentiment Check: Crucially, narrative sentiment MUST strictly match numerical data. Do NOT use positive idioms, celebratory language, or "green" framing for negative performance data. If context is DOWN or change_pct is negative, frame the narrative as a drop, struggle, or correction.
...
Verify: sentiment matches data sign — all facts match the input — no unsupported claims — tweet ≤280 characters.
...
def generate_tweet(symbol: str, slot: dict, price_ctx: dict,
    ...
    if price_ctx and not is_weekend():
        cur = price_ctx.get("currency_symbol", "$")
        raw_change = price_ctx["change_pct"]
        sign = "+" if raw_change >= 0 else ""
        direction = "UP" if raw_change >= 0 else "DOWN"
        
        parts = [f"Performance Context: {direction} ({sign}{raw_change}%)"]
        parts.append(f"Live: {cur}{price_ctx['price']}")
        if "day_high" in price_ctx and "day_low" in price_ctx:
...
_ANALYSIS_PIECE_PATTERNS = [
    ...
    # name in the headline is background color for the valuation model's opinion, not fresh
...
- Sentiment Check: Narrative sentiment MUST strictly match numerical data. If price change is negative, frame the reaction as a drop, sell-off, or correction. Do NOT use positive idioms (e.g., "ahead of the pack") for a DOWN stock.
...
## Review before output
Verify: facts match the input — sentiment matches data sign — no unsupported claims — tweet ≤280 characters.
...
## Rules
- Sentiment Check: Narrative sentiment MUST strictly match the raw performance context (provided via UP/DOWN labels or raw numbers). If change_pct is negative, frame the narrative as dropping, lagging, or correction. NEVER use celebratory language or bullish idioms (like "ahead of the pack") for negative numbers.
- NEVER invent or infer market conditions, catalysts, or facts not present in the provided data.
...
## Review before output
Verify: facts match data — narrative sentiment strictly matches numerical context sign.
...
def generate_market_update_tweet(key: str, ranked: list[str], ticker_data: dict,
    ...
    for t in ranked:
        ...
        if phase == "weekend":
            ...
            raw_change = ctx.get("change_pct", 0)
            cur  = ctx.get("currency_symbol", "$")
            sign = "+" if raw_change >= 0 else ""
            direction = "UP" if raw_change >= 0 else "DOWN"
            
            line = f"${base}: {cur}{ctx['price']} ({sign}{raw_change}%) [Direction: {direction}]"
